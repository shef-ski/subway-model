...
def create_sankey_diagram(data: pd.DataFrame, station_mapping: dict, hour: int):
    ...
    fig.update_layout(
        title_text=f"Sankey Diagram of Subway Ridership — Hour {hour}", font_size=10
    )
    return fig
...
app.layout = html.Div(
    [
        ...
        dcc.Dropdown(
            id="month-dropdown",
            options=[
                {"label": f"Month {int(month)}", "value": int(month)}
                for month in months
            ],
        ...
        dcc.Dropdown(
            id="hour-dropdown",
            options=[
                {"label": f"Hour {hour}", "value": hour} for hour in range(1, 25)
            ],  # Assuming 24 hours
            value=1,  # Default value
            clearable=False,
        ),
...
def update_sankey(selected_month, selected_day, selected_hour):
    ...
    subset = (
        subset.groupby(["origin", "destination", "hour_of_day"]).sum().reset_index()
    )
