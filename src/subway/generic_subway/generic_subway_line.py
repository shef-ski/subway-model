...
class GenericSubwayLine(AbstractSubwayLine, ABC):
    ...
    def __init__(self, name: str, n_stations: int, cap):
        ...
        self.train_travel_times = {
            station: {1: TRAVEL_TIME_BETWEEN_STATIONS, -1: TRAVEL_TIME_BETWEEN_STATIONS}
    ...
    def sample_arriving_passengers(
        self, station: SubwayStation, events: List[Event]
        ...
        if random.random() < GenericSubwayLine.p_arrival_in_a_second:
            ...
            passenger = SubwayPassenger(
                entry_id=station.id,
                leave_id=destination_station.id,
                direction=direction,
            )
    ...
    def check_for_train_spawns(self, current_time: datetime):
        if (
            current_time.minute % 4 == 0
            and current_time.second == 0
            and len(self.get_trains()) < 5
        ):
