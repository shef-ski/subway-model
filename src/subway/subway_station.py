...
class SubwayStation:
    ...
    def increase_waiting_passengers(
        self, disembarked_passengers: List[SubwayPassenger]
    ):
    ...
    def get_waiting_psg_for_train(
        self, remaining_stations_for_train: List[int]
    ) -> List[SubwayPassenger]:
        return [
            passenger
            for passenger in self.waiting_passengers
            if passenger.leave_id in remaining_stations_for_train
        ]

    def remove_waiting_passengers(
        self, passengers_entering_train: List[SubwayPassenger]
    ):
        self.waiting_passengers = [
            passenger
            for passenger in self.waiting_passengers
            if passenger not in passengers_entering_train
        ]
    ...
    def get_waiting_psg_up_regular(self) -> List[SubwayPassenger]:
        return [
            passenger
            for passenger in self.waiting_passengers
            if passenger.direction == 1 and passenger.event_name is None
        ]
    ...
    def get_waiting_psg_up_event(self) -> List[SubwayPassenger]:
        return [
            passenger
            for passenger in self.waiting_passengers
            if passenger.direction == 1 and passenger.event_name is not None
        ]
