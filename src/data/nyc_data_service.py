...
class NycDataService:
    ...
    STOPS_PATH = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../data/line_data/stops.txt")
    )
    SHAPE_PATH = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "../../data/nyc_map/CHS_UHF_34_DOHMH_2004.shp"
        )
    )
    ...
    def __init__(self):
        ...
        self.data_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "../../data/line_outputs")
        )

    def _line_data_exists(self, name):
        folder_path = os.path.join(self.data_path, name)
        required_files = [
            NycDataService.ESTIMATES_FILE_NAME,
            NycDataService.META_DATA_FILE_NAME,
        ]
        ...
        if missing_files:
            print(
                f"Folder '{name}' is missing the following files: {', '.join(missing_files)}"
            )
            return False
    ...
    def load_nyc_line(self, name: str, capacity: int = 500) -> NycSubwayLine:
        ...
        return NycSubwayLine(
            name, stations, lookup_table, train_spawns, train_travel_times, capacity
        )
    ...
    def read_train_spawns(
        self, name: str, stations: List[SubwayStation]
    ) -> Dict[Tuple[str, int, int, int], SubwayStation]:
        result = {}
        ...
        with open(self._get_train_arrival_file(name), newline="") as csvfile:
            ...
            for row in reader:
                ...
                if direction not in seen_directions:
                    ...
                    station_ids_in_trip = [
                        int(r["station_id"]) for r in direction_rows[direction]
                    ]
